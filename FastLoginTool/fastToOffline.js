...
$(function () {
    ...
    function checkPageType() {
        ...
        if (document.location.pathname == '/cii/promptmessage/Readmessage.asp' ||    //fat,uat,prod的当前uid的提示信息页面
            document.location.pathname == '/caso/login' ||                           //讨厌的统一登录平台
            document.location.pathname == '/CII/share/messagebox.asp')	             //登录出错的提示页面，烦躁，我关，我关，我关关关！                 
    ...
    function login() {
        switch (curEnvir) {
            case EnumEnvir.Fat:
                ...
                break;
            case EnumEnvir.Uat:
                document.getElementsByName('eid')[0].value = uatConfigs.eid;
                document.getElementsByName('pwd')[0].value = uatConfigs.ekey;
                document.getElementById('Submit1').click();
                break;
            case EnumEnvir.Prod:
                var isFirstLogin = document.getElementsByName('eid')[1] != null;
                var isSecondLogin = document.getElementsByName('logineid')[0] != null;
                if (isFirstLogin) {
                    document.getElementsByName('eid')[1].value = prodConfigs.eid;
                    document.getElementsByName('pwd')[1].value = prodConfigs.ekey;
                    document.getElementsByName('go2')[1].click();
                } else if (isSecondLogin) {
                    document.getElementsByName('logineid')[0].value = prodConfigs.eid;
                    document.getElementsByName('loginpwd')[0].value = prodConfigs.ekey;
                    document.getElementsByName('btnLogin')[0].click();
                } else {
                    //以后若还有第三次登录，那真是要骂娘了~！
                }
